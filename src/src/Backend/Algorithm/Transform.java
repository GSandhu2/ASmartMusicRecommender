...
public class Transform {
    ...
    public Transform(Reader audio) {
        ...
        // Perform transform
        leftFrequencyAmplitudes = cqt(audio.getChannel(Channel.LEFT), timeSamples, audio.getSampleRate());
        if (audio.getMode() == Reader.Mode.STEREO)
            rightFrequencyAmplitudes = cqt(audio.getChannel(Channel.RIGHT), timeSamples, audio.getSampleRate());
        else
    ...
    private static float[][] cqt(short[] audioSamples, int samples, int sampleRate) {
        ...
        for (int j = 0; j <= FREQUENCY_RESOLUTION; j++) {
            double frequency = BOTTOM_FREQUENCY * Math.pow(TOP_BOTTOM_RATIO, (double)j / FREQUENCY_RESOLUTION);
    ...
    private static float transform(short[] audioSamples, int start, int end, double frequency, int sampleRate) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Left channel frequency analysis (window length = " + (reader.getSampleRate() / (int)TIME_RESOLUTION) + " samples):");

            System.out.print("    Frequencies:");
            for (int i = 0; i <= FREQUENCY_RESOLUTION; i++)
                System.out.print(" " + String.format("%8s", format.format(BOTTOM_FREQUENCY * Math.pow(TOP_BOTTOM_RATIO, (double) i / FREQUENCY_RESOLUTION))));
            System.out.println();

            float[][] left = transform.getFrequencyAmplitudes(Channel.LEFT);
            for (int i = 0; i < left.length; i++) {
                System.out.print("At time " + String.format("%8s", format.format(i / TIME_RESOLUTION) + ":"));
